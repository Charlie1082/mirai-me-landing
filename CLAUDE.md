...
> **필수 참조**: AI 조직 운영 규칙은 `/Users/charlie/Workshop/MIRAI-ME/AI_ORGANIZATION.md` 참조
> 모든 부서 출력물은 단독 모델 금지, 최소 2모델 협업 + 총괄(Claude) 검증 필수

## 프로젝트 폴더 구조 (필수 준수)

MIRAI-ME 프로젝트의 모든 작업은 반드시 `/Users/charlie/Workshop/MIRAI-ME/` 폴더 내에서 수행한다.
...
1. **절대 경로**: 랜딩 페이지 개발은 `/Users/charlie/Workshop/MIRAI-ME/03_개발/landing/`에서만 진행
2. **중복 금지**: E: 루트나 다른 경로에 프로젝트 복제본을 생성하지 않는다
3. **새 모듈**: 새로운 개발 모듈(앱, API 등)은 `/Users/charlie/Workshop/MIRAI-ME/03_개발/` 하위에 생성
4. **문서/기획**: 기획 문서는 `01_기획/`, 디자인은 `02_디자인/` 등 해당 폴더에 저장
5. **Git 작업**: `cd /Users/charlie/Workshop/MIRAI-ME/03_개발/landing` 후 git 명령 실행
